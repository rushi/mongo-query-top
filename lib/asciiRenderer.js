...
export default class AsciiRenderer {
    ...
    renderHeader(connections = null) {
        ...
        if (connections) {
            titles.push(`Connections: ${bold.cyan(`${connections.current}/${connections.available}`)}`);
        }

        titles.push(c.gray(`Window Size: ${JSON.stringify(width)}x${JSON.stringify(height)}`));
    ...
    renderBody(queries) {
        ...
        _.each(filteredQueries, q => {
            ...
            if (query.effectiveUsers) {
                row.opid += `\n${c.blue(query.effectiveUsers.map(e => Object.values(e).join("@")))}`;
            }
        ...
        data.forEach(row => {
            ...
            if (row.q.planSummary && row.q.planSummary === "COLLSCAN") {
                ...
                this.saveQuery(row.q, row.q.ns, "COLLSCAN"); // Log all un-indexed queries
            }
