...
class Renderer
{
    ...
    renderBody(queries)
    {
        ...
        let lineCount = 0;
        _.each(queries, q => {
            const query = q.command.pipeline ? q.command : q.command.query ? q.command.query : q.command.filter;            
            let row = {
                q: q,
                idx: idx++,
    ...
    shouldSkipQuery(q)
    {
        ...
        return  q.command && q.command.filter && q.op == 'command' &&  q.ns == 'admin.$cmd' || 
