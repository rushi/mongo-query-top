...
export default class Renderer {
    ...
    renderHeader() {
        const titles = [
            ...
            `Time: ${chalk.white.bold(dayjs(new Date()).format('MMM DD HH:mm:ss'))}`,
    ...
    renderBody(queries) {
        const colWidths = [5, 20, 10, 25]; // #, ID, Age, ns, op
        ...
        _.each(queries, q => {
            ...
            let appName;
            if (q.clientMetadata) {
                if (q.clientMetadata.driver?.name) {
                    row.opid += `\n${chalk.grey(q.clientMetadata.driver.name)}`;
                }
                if (q.clientMetadata.application?.name) {
    ...
    save(queries) {
        ...
        if (filteredQueries.length > 0) {
            const filename = `mongodb-query-top-snapshot-${dayjs().format()}.json`;
