...
class Renderer
{
    ...
    renderBody(queries)
    {
        ...
        _.each(queries, q => {
            let row = {
                ...
                selectedQuery: beautify(q.command.filter, null, 2, maxQueryLength)
            };
        ...
        for (let d in data) {
            ...
            lineCount = (lineCount - row.selectedQueryLength) + (row.opid) ? row.opid.split('\n').length : '';
            modifiedCount++;
        }
        ...
        if (idx === 1) {
            table.push([{content: 'No queries running, great!', colSpan: 5}]);
        }
    ...
    shouldSkipQuery(q)
    {
        ...
        return q.command && q.command.filter && q.op == 'command' &&  q.ns == 'admin.$cmd' || q.ns == 'local.oplog.rs' || (q.op == 'none' && !isIndex) || q.secs_running < this.prefs.minTime;
    }
}
